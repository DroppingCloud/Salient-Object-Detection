...
class FoldConvASPP(nn.Module):
    """将特征图 unfold 成 patch，在 patch 空间做多尺度空洞卷积，再 fold 回原分辨率"""
...
class GateModule(nn.Module):
    """encoder+decoder 特征 concat → conv → global avg pool sigmoid → (B, 2, 1, 1)"""
...
class GateNet(nn.Module):
    ...
    def __init__(self, pretrained=True, backbone_name="resnet18"):
        ...
        # 最深层使用 FoldConvASPP，其余用普通 3×3 conv
        ...
        # ── Gate Modules (encoder + 上级 decoder 拼接) ───────────────
        # FPN 各级输出通道与下一级 T 通道对齐
        d4_ch = t3_ch
        d3_ch = t2_ch
        d2_ch = t1_ch
        ...
        # ── Parallel Branch: 聚合 gate[1]*T 各级被抑制特征做残差修正 ──
    ...
    def forward(self, x):
        ...
        D4 = self.fpn4(G4[:, 0:1] * T4)                     # gate[0] 前景分支
        ...
        # ── Parallel Branch (gate[1]*T 各级上采样拼接，残差修正) ──
